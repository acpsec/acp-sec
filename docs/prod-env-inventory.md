...
| `B20_GETLOGS_CHUNK_8453` | Fallback `eth_getLogs` chunk size (blocks) for the b20 role/announcement replay on Base **mainnet** (8453) — the window size used when the provider rejects a full-range query. Unset ⟹ today's size (`10000`). | no | optional | **Recommended `100000` on Coinbase CDP** (its getLogs range cap) to walk in ~10× fewer, wider windows. Set to whatever `B20_RPC_URL_8453`'s provider allows; zero-config = today's public-endpoint size. Completes the per-chain pair: `B20_RPC_URL_*` picks the provider, `B20_GETLOGS_CHUNK_*` tunes to its range policy. |
| `B20_GETLOGS_CHUNK_84532` | Same as `B20_GETLOGS_CHUNK_8453`, for Base **Sepolia** (84532). Unset ⟹ today's size (`2000`). | no | optional | Match the testnet provider's getLogs range cap. |
